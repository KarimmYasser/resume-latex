...
OUT_DIR="${SCRIPT_DIR}/Tailored_CVs"
...
for file in "${SCRIPT_DIR}"/tailored_cv_content/resume-content-*.tex; do
    [ -e "$file" ] || continue
    ...
    sed "s|\\\\input{resume-content\.tex}|\\\\input{tailored_cv_content/${filename}}|" "${SCRIPT_DIR}/resume.tex" > "${temp_tex}"
