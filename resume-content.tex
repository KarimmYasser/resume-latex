...
\newcommand{\projectsList}{%
    ...
    {Post-Quantum Cryptographic Research: Constant Hamming Weight Encoding}%
    {Conducted a deep technical analysis and reproduction of a \textbf{Constant-Time Encoding} primitive for \textbf{Code-Based PQC} (Classic McEliece, BIKE, HQC). Evaluated deterministic parameter selection ($d_{\min}$) using \textbf{power-of-two optimization} to eliminate expensive binomial computations. Implemented and verified \textbf{Branchless Arithmetic} (\texttt{IsZero}-masking) to achieve \textbf{Side-Channel Resistance}, ensuring stable timing across secret-dependent inputs. Validated implementation security via \textbf{Welch’s t-test} ($|t| < 4.5$) and exhaustive testing on \textbf{MicroBlaze (FPGA)} and \textbf{Intel x86} architectures.}%
    |||
    {Mar 2026}%
...
Post-Quantum Constant-Time Programming,
